...
public class XPathParser {
	...
	private void consumeOperatorFunction() {
		...
		} else if (remainder.startsWith("concat(")) {
			functionConcat(remainder);
	...
	private void functionConcat(String remainder) {
		Validate.isTrue(remainder.endsWith(")"), "Unclosed bracket for function! " + remainder);
		List<String> params = XTokenQueue.trimQuotes(
				XTokenQueue.parseFuncionParams(remainder.substring("concat(".length(), remainder.length() - 1)));
		elementOperator = new ElementOperator.ConcatText(params);			
